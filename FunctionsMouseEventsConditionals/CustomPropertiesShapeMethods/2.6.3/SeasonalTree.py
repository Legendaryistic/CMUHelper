...
def onMousePress(mouseX, mouseY):
    dot = Circle(mouseX, mouseY, 1, visible = False)
    
    if dot.hitsShape(springLabel):
        ground.visible = False
        leaves.fill = 'lightPink'
    elif dot.hitsShape(summerLabel):
        ground.visible = False
        leaves.fill = 'forestGreen'
    elif dot.hitsShape(fallLabel):
        leaves.fill = 'darkOrange'
        ground.visible = True
        ground.fill = 'fireBrick'
    elif dot.hitsShape(winterLabel):
        leaves.fill = 'white'
        ground.visible = True
        ground.fill = 'white'
