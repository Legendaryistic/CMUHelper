...
def drawOrange(cx, cy, o):
    o.add(
        Circle(cx, cy, 20, fill='khaki', border='darkOrange'),
...
def onMousePress(mouseX, mouseY):
    # Checks if we clicked on any fruit. If so, draws a copy in the top of the
    # blender and adds that copy to the newFruit group.
    newFruit = Group()
    
    
    if (orange.contains(mouseX, mouseY) == True):
        drawOrange(275, 55, newFruit)
        app.fruitsToBlend.append(newFruit.juiceColor)
        app.fruitGroups.append(newFruit)
    ...
    if powerButton.contains(mouseX, mouseY):
        for fruit in app.fruitsToBlend:
            Rect(175,210,65,90, fill = fruit, opacity = 30)
        funnel.toFront()
        ...
        for i in range(len(app.fruitsToBlend)):
            app.fruitsToBlend.pop(0)
        for group in app.fruitGroups:
            for item in group.children:
                item.visible = False
            
        
    # When the newFruit group contains a fruit, add the entire group to the
    # fruitsToBlend list and set the funnel to be in front of it on the canvas.
    ### Place Your Code Here ###

    # When the power button is clicked on, blend all fruits in the
    # app.fruitsToBlend list by clearing them. Then draw juice in the cup
    # with an appropriate opacity and color.
    ### (HINT: If more than one fruit is blended, the cup should have one
    #          rectangle of juice for each fruit.)
    ### (HINT: Each fruit has a custom .juiceColor property.)
    ### Place Your Code Here ###
