...
def makeSushi(color):
    ...
    piece = Group (
        ...
        Line(50, 100, 75, 150, fill=rgb(240, 240, 240),
             lineWidth=45, dashes=(2, 6)),
...
def onMousePress(mouseX, mouseY):
    if salmon.contains(mouseX, mouseY):
        makeSushi('coral')
    elif tuna.contains(mouseX, mouseY):
        makeSushi('crimson')
    elif yellowTail.contains(mouseX, mouseY):
        makeSushi('lightCoral')
    else:
        if len(app.sushiOrder) != 0:
            app.sushiOrder[-1].visible = False
            app.sushiOrder.pop(-1)
