...
def drawPetalRing(numberOfPetals, x, y):
    ...
    for i in range(numberOfPetals):
        ...
        if app.indexer%2 == 0:
            app.color = app.lightColor
        else:
            app.color = app.darkColor
        ...
        Flower = Rect(x-width/2, y-height/2, width, height, fill = app.color)
        Flower.rotateAngle = angle
        app.indexer += 1
def onMousePress(mouseX, mouseY):
    ...
    if (roseBush.hits(mouseX, mouseY) == True):
        getRoseColors()
        for x in numberOfPetalsInRing:
            drawPetalRing(x, mouseX, mouseY)
